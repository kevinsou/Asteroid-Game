...
public class GameWorld extends Observable implements IGameWorld {
		...
		while(theElements.hasNext()){
			...
			if(obj instanceof PlayerShip) {
				if(((PlayerShip)obj).getMissileCount()>=0) {
					...
					if(soundOn) {
						Sound sound = new Sound ("bark.WAV");
						sound.play();
					}
					this.setChanged();
					this.notifyObservers(new GameWorldProxy(this));
	...
	public void tick() {
		...
		while(theElements.hasNext()) {
			ICollider curObj = (ICollider)theElements.next();
			Iterator<Object> testElements = go.getIterator();
			try {
			while(testElements.hasNext()) {
				ICollider otherObj = (ICollider)testElements.next();
				if(otherObj != curObj) {
					//System.out.println("test");
					if(curObj.collidesWith(otherObj)) {
						//System.out.println("test");
						curObj.handleCollision(otherObj);
						if(((GameObject) curObj).getCollision().equals("psCrashAsteroid")){
							//System.out.println("test");
							testElements.remove();
							//System.out.println("test");
							theElements.remove();
							...
							break;
							//psCrashAsteroid();
						}else if(((GameObject) curObj).getCollision().equals("asteroidCollided")) {
							//asteroidsCollided();
							testElements.remove();
							theElements.remove();
							break;
						}else if(((GameObject) curObj).getCollision().equals("asteroidColliededNPS")) {
							//asteroidColliededNPS();
							testElements.remove();
							theElements.remove();
							break;
						}else if(((GameObject) curObj).getCollision().equals("playerMissileKillAsteroid")) {
							//playerMissileKillAsteroid();
							testElements.remove();
							theElements.remove();
							if(soundOn) {
								Sound sound = new Sound("Cat.WAV");
								sound.play();
							}
							score = score+500;
							break;
						}else if(((GameObject) curObj).getCollision().equals("psHitNPS")) {
							//psHitNPS();
							testElements.remove();
							theElements.remove();
							lives--;
							break;
						}else if(((GameObject) curObj).getCollision().equals("playerMissileEliminateNPS")) {
							testElements.remove();
							theElements.remove();
							score = score +1000;
							break;
						}else if(((GameObject) curObj).getCollision().equals("npsStruckPS")) {
							testElements.remove();
							theElements.remove();
							lives--;
							break;
						}else if(((GameObject) curObj).getCollision().equals("reload")) {
							reload();
							break;
						}    
					}
		...
		if(lives > 0) {
			...
			if(soundOn) {
				Sound sound = new Sound ("donkey2.wav");
				sound.play();
				Dialog.show("Final Score :" , Integer.toString(score), "Quit", "Play Again?");
			}
		}
		gameclock++;
		this.setChanged();
...
//		System.out.println();
//	}
	public void setPlayOn(boolean b) {
		this.playOn=b;
	}
